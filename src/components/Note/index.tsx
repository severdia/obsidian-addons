...
interface NoteProps {
  ...
  isFirst?: boolean;
  notePosition: number;
  isKeyboardSelected?: boolean; // new
}

export const Note = memo(
  ({ file, isFirst, notePosition, isKeyboardSelected }: NoteProps) => {
    ...
    const backgroundListColorClass = (() => {
      if (isKeyboardSelected && !isSelected) {
        return "onb-bg-[--onb-note-keyboard-highlight] onb-rounded-md onb-z-10";
      }
